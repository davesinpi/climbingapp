...
const SessionLogger: React.FC<SessionLoggerProps> = ({ session: initialSession, onComplete, onCancel }) => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] lg:h-[calc(100vh-80px)] bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
      {/* Fixed Header */}
      <div className="flex-none bg-indigo-600 text-white p-6 flex justify-between items-center z-30 shadow-md">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col">
        {/* Sub-nav (Exercises Tab) - Sticky at the top of scrollable area */}
        <div className="sticky top-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-20 px-6 py-4 border-b border-slate-100 dark:border-slate-800">
                ...
                {currentExercise.sets.map((set, sIdx) => (
                  <div key={set.id} className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl relative group border border-slate-100 dark:border-slate-800 hover:border-indigo-200 transition-colors">
                    ...
                    <button 
                      ...
                      className="absolute -top-2 -right-2 w-8 h-8 bg-white dark:bg-slate-700 shadow-md border border-slate-100 dark:border-slate-600 rounded-full flex items-center justify-center text-slate-300 hover:text-red-500 transition-colors lg:opacity-0 group-hover:opacity-100"
                    >
      ...
      {/* Fixed Footer */}
      <div className="flex-none bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 p-6 flex items-center justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.05)] z-30">
